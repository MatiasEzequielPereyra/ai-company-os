...
function Test-TransientXaiError {
...
if ([string]::IsNullOrWhiteSpace($env:XAI_API_KEY)) {
    throw "XAI_API_KEY is not configured."
}
...
catch { throw "xAI request payload is invalid JSON before transport: $($_.Exception.Message)" }
...
for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
    try {
        ...
        $isTransient = Test-TransientXaiError -ErrorRecord $_ -StatusCode $statusCode
        if ($isTransient -and $attempt -lt $maxAttempts) {
            $delaySeconds = [Math]::Pow(2,($attempt - 1))
            Write-Host ("xAI transient failure on attempt " + $attempt + "/" + $maxAttempts + ". Retrying in " + $delaySeconds + "s...") -ForegroundColor Yellow
            Start-Sleep -Seconds $delaySeconds
            continue
        }

        throw "xAI request failed: $message"
    }
}

if ($null -eq $response) {
    throw "xAI request failed without a response after $maxAttempts attempts."
}

if ($null -eq $response.choices -or $response.choices.Count -lt 1) {
    throw "xAI returned no completion choices."
}

$content = ([string]$response.choices[0].message.content).Trim()
if ([string]::IsNullOrWhiteSpace($content)) {
    throw "xAI returned empty completion content."
}
...
try {
    ...
    throw "xAI returned invalid JSON for the structured result contract."
}
