...
# Add high-value file contents before spending budget on the inventory listing.
foreach ($entry in ($allFiles | Sort-Object @{Expression="Score";Descending=$true}, @{Expression="Relative";Descending=$false})) {
    if ($used -ge ($MaxChars - 30000)) { break }

    $key = $entry.Relative.ToLowerInvariant()
    if ($included.ContainsKey($key)) { continue }

    $added = Add-ContextFile -Builder $builder -Root $root -RelativePath $entry.Relative -Remaining (($MaxChars - 30000) - $used)
    if ($added -gt 0) {
        $included[$key] = $true
        $used += $added
    }
